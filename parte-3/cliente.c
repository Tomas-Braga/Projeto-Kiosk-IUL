 ...
 ** Este módulo é responsável por implementar o processo Cliente.
 ** Pede os dados ao utilizador, envia-os para o Servidor, recebe a lista de produtos e envia o pedido de compra.
 ******************************************************************************/
#include "common.h"
...
int initMsg_C1() {
    ...
    so_debug("<");
    //Abre a Message Queue (MSG) do projeto, que tem a KEY IPC_KEY
    result = msgGet();
    if(result < 0){
        so_error("C1", "");
        so_exit_on_error(-1, "");
    } else {
        so_success("C1", "%d", result);
    }
...
int triggerSignals_C2() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Arma e trata os sinais SIGUSR2 (ver C10), SIGINT (ver C11), e SIGALRM (ver C12).
    if(signal(SIGUSR2, trataSinalSIGUSR2_C10) == SIG_ERR || signal(SIGINT, trataSinalSIGINT_C11) == SIG_ERR || signal(SIGALRM, trataSinalSIGALRM_C12) == SIG_ERR){
        so_error("C2", "");
        so_exit_on_error(-1, "");
    } else {
        so_success("C2", "");
...
MsgContent getDadosPedidoUtilizador_C3() {
    ...
    //criando um elemento do tipo Login com essas informações, e preenchendo também o campo pidCliente
    //com o PID do seu próprio processo Cliente. Os restantes campos da estrutura Login não precisam ser preenchidos.
    //Em caso de qualquer erro, dá so_error e termina o Cliente.
    //Caso contrário dá so_success <nif> <senha> <pidCliente>;
    //Convenciona-se que se houver problemas, esta função coloca nif = USER_NOT_FOUND;
    printf("NIF: ");
    scanf("%d", &msg.msgData.infoLogin.nif);
    printf("Senha: ");
    scanf("%s", msg.msgData.infoLogin.senha);
    msg.msgData.infoLogin.pidCliente = getpid();
    if(msg.msgData.infoLogin.nif == 0 || msg.msgData.infoLogin.senha == NULL){
        msg.msgData.infoLogin.nif = USER_NOT_FOUND;
        so_error("C3", "");
        so_exit_on_error(-1, "");
    }
...
int sendClientLogin_C4(MsgContent msg) {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Envia uma mensagem do tipo MSGTYPE_LOGIN para a MSG com a informação recolhida do utilizador.
...
void configuraTemporizador_C5(int tempoEspera) {
    ...
    //Configura um alarme com o valor de tempoEspera
    alarm(tempoEspera);
...
int receiveProductList_C6() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Desliga o alarme configurado em C5
    alarm(0);
    ...
    if(msg.msgData.infoLogin.pidServidorDedicado == -1){
        ...
        so_success("C6", "%s %d %d", msg.msgData.infoLogin.nome, msg.msgData.infoLogin.saldo, msg.msgData.infoLogin.pidServidorDedicado);
        printf("Lista de Produtos Disponíveis:\n");
    }

    //Extrai da mensagem recebida o Produto especificado
    if(msg.msgData.infoProduto.idProduto != FIM_LISTA_PRODUTOS){
        //Mostra no STDOUT uma linha de texto com a indicação de idProduto, Nome, Categoria e Preço
        printf("%d %s %s %d\n", msg.msgData.infoProduto.idProduto, msg.msgData.infoLogin.nome, msg.msgData.infoProduto.categoria, msg.msgData.infoProduto.preco);

        //Lê da Message Queue uma nova mensagem cujo tipo é o PID deste processo Cliente
        if(msgReceive(msgId, &msg, getpid()) < 0){
            so_error("C6.5", "");
            so_exit_on_error(-1, "");
        } else {
            so_success("C6.5", "%d %s %s %d", msg.msgData.infoProduto.idProduto, msg.msgData.infoLogin.nome, msg.msgData.infoProduto.categoria, msg.msgData.infoProduto.preco);
        }
    } else {
        so_success("C6.3", "");
        result = RET_SUCCESS;
...
int getIdProdutoUtilizador_C7() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Pede ao utilizador que indique qual o idProduto (número) que deseja adquirir
    printf("Indique o idProduto que deseja adquirir: ");
    scanf("%d", &result);
    //Em caso de qualquer erro, dá so_error e retorna PRODUCT_NOT_FOUND
    if(result < 0){
        so_error("C7", "");
...
int sendClientOrder_C8(int idProduto, int pidServidorDedicado) {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Envia uma mensagem cujo tipo é o PID do Servidor Dedicado para a MSG com a informação do idProduto recolhida do utilizador
...
void receivePurchaseAck_C9() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Lê da MSG uma mensagem cujo tipo é o PID deste processo Cliente, com a resposta final do Servidor Dedicado
    if(msgReceive(msgId, &msg, getpid()) < 0){
        so_error("C9", "");
    } else {
    ...
    //Se o campo idProduto for PRODUTO_COMPRADO, escreve no STDOUT “Pode levantar o seu produto”
