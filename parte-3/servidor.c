 ...
 **
 ** Aluno: Nº:Tomás Rafael Freitas Braga       Nome:93024
 ** Nome do Módulo: servidor.c v2 (https://moodle22.iscte-iul.pt/mod/forum/discuss.php?d=6117#p10339)
 ** Descrição/Explicação do Módulo:
 ** Este módulo é responsável por criar o processo Servidor e os processos Servidor Dedicado.
 ** O processo Servidor é responsável por criar os processos Servidor Dedicado, e por receber os pedidos dos clientes.
 ** Realiza a autentição dos clientes, e envia a lista de produtos disponíveis.
 ** O processo Servidor Dedicado é responsável por receber os pedidos dos clientes, e por enviar as respostas aos clientes.
 ** Informa o cliente se o seu pedido foi aceite ou não e termina.
 ******************************************************************************/
#include "common.h"
...
int initShm_S1() {
    ...
    so_debug("<");
    //Tenta abrir a Shared Memory IPC com a referida KEY IPC_KEY
    shmId = shmGet();
    if (shmId != -1) {
        db = (DadosServidor*) shmAttach(shmId); 
        if (db != NULL) {
            result = shmId;
    ...
    //Valida se o erro anterior foi devido a não existir ainda nenhuma SHM criada com essa KEY
    if (errno == ENOENT) {
        so_success("S1.2", "");
    } else {
        so_error("S1.2", "");
    ...
    //Cria uma Shared Memory com a KEY IPC_KEY definida em common.h e com o tamanho para conter as duas listas do Servidor e liga a variável db a essa SHM
    shmId = shmCreate(IPC_KEY);
    if (shmId != -1) {
        db = (DadosServidor*)shmAttach(shmId);
        if (db != NULL) {
    ...
    //Inicia a Lista de Utilizadores, preenchendo em todos os elementos o campo nif=USER_NOT_FOUND, e a Lista de Produtos, preenchendo em todos os elementos o campo idProduto=PRODUCT_NOT_FOUND
    for (int i = 0; i < MAX_USERS; i++) {
    ...
    //Lê o ficheiro FILE_DATABASE_USERS e preenche a lista de utilizadores na memória partilhada com a informação dos utilizadores, mas preenchendo sempre os campos pidCliente e pidServidorDedicado com o valor -1
    FILE *fileUsers = fopen(FILE_DATABASE_USERS, "rb");
    if (fileUsers != NULL) {
        int numUsers = 0;
        while (fread(&db->listUsers[numUsers], sizeof(Login), 1, fileUsers) == 1) {
            db->listUsers[numUsers].pidCliente = -1;
            db->listUsers[numUsers].pidServidorDedicado = -1;
            numUsers++;
    ...
    //Lê o ficheiro FILE_DATABASE_PRODUCTS e preenche a lista de produtos na memória partilhada com a informação dos produtos
    FILE *fileProducts = fopen(FILE_DATABASE_PRODUCTS, "rb");
    if (fileProducts != NULL) {
        int numProducts = 0;
        while (fread(&db->listProducts[numProducts], sizeof(Produto), 1, fileProducts) == 1) {
            numProducts++;
...
int initMsg_S2() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Se já existir, deve apagar a fila de mensagens
    if (msgGet() != -1) {
        if (msgRemove(msgId) != -1) {
            so_success("S2.1", "");
        } else {
            so_error("S2.1", "");
            result = RET_ERROR;
    ...
    //Cria a Message Queue com a KEY IPC_KEY
    msgId = msgCreate();
    if (msgId != -1) {
...
int initSem_S3() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Cria um grupo de semáforos que tem a KEY IPC_KEY
    if (semGet() != -1) {
    ...
    //Cria um grupo de três semáforos com a KEY IPC_KEY
    semId = semCreate(3);
    ...
    //Inicia o valor dos semáforos SEM_USERS e SEM_PRODUCTS para que possam trabalhar em modo “exclusão mútua”, e inicia o valor do semáforo SEM_NR_SRV_DEDICADOS com o valor 0
    if (semNrSetValue(semId, SEM_USERS, 1) == 0 && semNrSetValue(semId, SEM_PRODUCTS, 1) == 0 && semNrSetValue(semId, SEM_NR_SRV_DEDICADOS, 0) == 0) {
        so_success("S3.3", "");
        result = semId;
    } else {
        so_error("S3.3", "");
...
int triggerSignals_S4() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Arma e trata os sinais SIGINT e SIGCHLD
...
void shutdownAndExit_S7() {
    ...
    //Verifica se existe SHM aberta e alocada
    if (shmGet() != RET_ERROR) {
        so_success("S7.1", "");
    } else {
        so_error("S7.1", "");
    ...
    //Percorre a lista de utilizadores e envia ao PID desse Servidor Dedicado o sinal SIGUSR1. Quando tiver terminado, dá so_success
    for (int i = 0; i < MAX_USERS; i++) {
    ...
    //Reescreve o ficheiro bd_produtos.dat incluindo apenas os produtos existentes na lista de produtos que tenham stock > 0
    FILE *fp = fopen("bd_produtos.dat", "w");
    if (fp == NULL) {
        so_error("S7.4", "");
    } else {
        for (int i = 0; i < MAX_PRODUCTS; i++) {
            if (db->listProducts[i].stock > 0) {
                fwrite(&db->listProducts[i], sizeof(Produto), 1, fp);
            }
        }
        fclose(fp);
        so_success("S7.4", "");
    ...
    //Apaga todos os elementos IPC (SHM, SEM e MSG) que tenham sido criados pelo Servidor com a KEY IPC_KEY
    if (shmId != RET_ERROR) {
        if (shmRemove() == RET_ERROR) {
            so_error("S7.5", "");
        } else {
            so_success("S7.5", "");
        }
...
void handleSignalSIGINT_S8(int sinalRecebido) {
    ...
    //Se receber esse sinal (do utilizador via Shell), o Servidor dá so_success, e vai para o passo terminal S7
    so_success("S8", "");
    shutdownAndExit_S7();
    so_debug(">");
}
...
void handleSignalSIGCHLD_S9(int sinalRecebido) {
    so_debug("< [@param sinalRecebido:%d]", sinalRecebido);
    //Se o Servidor receber esse sinal, identifica o PID do Servidor Dedicado que terminou (usando wait)
    int pidServidorDedicado = wait(NULL);
    so_success("S9", "Terminou Servidor Dedicado %d", pidServidorDedicado);
    so_debug(">");
}
...
int triggerSignals_SD10() {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("<");
    //Servidor Dedicado (filho) arma os sinais SIGUSR1 (ver SD18) e SIGINT (programa-o para ignorar este sinal)
...
int validateRequest_SD11(Login request) {
    ...
    //Valida, em primeiro lugar, no pedido Login recebido do Cliente (herdado do processo Servidor pai), se o campo pidCliente > 0
...
int searchUserDB_SD12(Login request) {
    ...
    for (int i = 0; i < MAX_USERS; i++) {
        //Procura pelo utilizador com o NIF e senha recebido no pedido do Cliente
        if (db->listUsers[i].nif == request.nif) {
            if (strcmp(db->listUsers[i].senha, request.senha) == 0) {
                indexClient = i;
                so_success("SD12.1", "%d", indexClient);
                reserveUserDB_SD13(indexClient, request.pidCliente);
            } else {
                so_error("SD12.1", "");
            }
        } else {
    ...
    //Cria uma resposta indicando erro ao Cliente 
    if (indexClient == USER_NOT_FOUND) {
        request.pidServidorDedicado = -1;
        MsgContent response;
        response.msgData.infoLogin = request;    
        
        if (msgSend(msgId, response, request.pidCliente) == RET_ERROR) {
            so_error("SD12.2", "");
            shutdownAndExit_SD17();
        } else {
            so_success("SD12.2", "");
...
int reserveUserDB_SD13(int indexClient, int pidCliente) {
    ...
    //Atualiza na Lista de Utilizadores, na posição indexClient, os campos pidServidorDedicado e pidCliente
...
int sendProductList_SD14(int indexClient, int pidCliente) {
    int result = RET_ERROR;    // Por omissão retorna erro
    so_debug("< [@param indexClient:%d, pidCliente:%d]", indexClient, pidCliente);
    //Preenche na mensagem de resposta os campos nome e saldo da estrutura Login com os valores da Lista de Utilizadores para indexClient
    MsgContent response;
    strcpy(response.msgData.infoLogin.nome, db->listUsers[indexClient].nome);
    response.msgData.infoLogin.saldo = db->listUsers[indexClient].saldo;
    response.msgData.infoLogin.pidServidorDedicado = getpid();
    so_success("SD14.1", "");

    //Percorre a Lista de Produtos e envia ao Cliente os produtos com stock > 0
    for (int i = 0; i < MAX_PRODUCTS; i++) {
        if (db->listProducts[i].stock > 0) {
            response.msgType = (long)pidCliente;
            response.msgData.infoProduto = db->listProducts[i];
            if (msgSend(msgId, response, 0) == -1) {
                so_error("SD14.2", "");
                shutdownAndExit_SD17();
            }
        }
    ...
    //Envia ao Cliente a mensagem de fim de lista
    response.msgType = (long)pidCliente;
    response.msgData.infoProduto.idProduto = FIM_LISTA_PRODUTOS;
    if (msgSend(msgId, response, 0) == -1) {
        so_error("SD14.3", "");
        result = RET_ERROR;
        shutdownAndExit_SD17();
...
MsgContent receiveClientOrder_SD15() {
    ...
    msg.msgData.infoLogin.nif = USER_NOT_FOUND;   // Por omissão retorna erro
    so_debug("<");
    if (msgReceive(msgId, &msg, (long)getpid()) == -1) {
        so_error("SD15", "");
        shutdownAndExit_SD17();
    } else {
        so_success("SD15", "");
 ...
 *        SD16.1    Se o idProduto enviado pelo Cliente for PRODUTO_NOT_FOUND, então preenche o
...
int sendPurchaseAck_SD16(int idProduto, int pidCliente) {
    ...
    //Se o idProduto enviado pelo Cliente for PRODUTO_NOT_FOUND, então preenche o campo idProduto=PRODUTO_NAO_COMPRADO
    if (idProduto == PRODUCT_NOT_FOUND) {
        idProduto = PRODUTO_NAO_COMPRADO;
        so_error("SD16.1", "");
    } else {
        //percorre a lista de produtos e verifica se o idProduto existe e se tem stock
        for (int i = 0; i < MAX_PRODUCTS; i++) {
            if (db->listProducts[i].idProduto == idProduto && db->listProducts[i].stock > 0) {
                db->listProducts[i].stock--;
                idProduto = PRODUTO_COMPRADO;
                so_success("SD16.2", "");
                result = RET_SUCCESS;               
            } else {
                idProduto = PRODUTO_NAO_COMPRADO;
    ...
    //Envia, usando como msgType o PID do processo Cliente, a mensagem de resposta de conclusão ao Cliente
    MsgContent response;
    response.msgType = (long)pidCliente;
    response.msgData.infoProduto.idProduto = idProduto;
    if (msgSend(msgId, response, 0) == -1) {
        so_error("SD16.3", "");
    } else {
        so_success("SD16.3", "");
...
void shutdownAndExit_SD17() {
    so_debug("<");
    so_success("Shutdown Servidor Dedicado", "");
    //Atualizar na Lista de utilizadores para a posição indexClient, os campos pidServidorDedicado=-1 e pidCliente=-1
...
void handleSignalSIGUSR1_SD18(int sinalRecebido) {
    so_debug("< [@param sinalRecebido:%d]", sinalRecebido);
    //Se o Servidor Dedicado receber esse sinal, envia um sinal SIGUSR2 ao Cliente
